...
titlevec = {'Willsky GLR','MGLR-RLS','MGLR-DUAL','Willsky-Single','Jamouli-AGLR','MGLR-Mest','Jamouli-AGLR, 2-stage'};
filtvec = {'Willsky','MGLR_RLS','MGLR-DUAL','MGLR-Mest'};
glrlegvec = {'$\epsilon_{\hat{x},Willsky}$','$\epsilon_{\hat{x},MGLR-RLS}$',...
    '$\epsilon_{\hat{x},MGLR}$','$\epsilon_{\hat{x},Willsky-1}$',...
    '$\epsilon_{\hat{x},AGLR}$','$\epsilon_{\hat{x},MGLR-Mest}$','$\epsilon_{\hat{x},AGLR,2-Stage}$'};
...
if kplotres
    for kp = kpvec;
        ...
        for kk = [7]
            ...
            ylabel('Position est. error [m]','interpreter','latex','fontsize',14)
...
% %%
%
% lbls = {'North [m/s]', 'East [m/s]', 'Down [m/s]'};
% figure('units','normalized','outerposition',[0 0 1 1]); set(gcf, 'Color', 'w');
% for kp = [1 2 3]
%     ky = [2 4 6];
%     subplot(3,1,kp),
%     plot(z_out.time, z_out.signals.values(:,ky(kp)),'k','linewidth',2)
%     ylabel(lbls(kp),'interpreter','latex')
%     axis tight
% %     xlabel('Time [s]','interpreter','latex')
%     funFigureProperty
% end